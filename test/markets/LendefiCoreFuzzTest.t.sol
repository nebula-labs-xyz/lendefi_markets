...
contract LendefiCoreFuzzTest is BasicDeploy {
    ...
    function setUp() public {
        ...
        marketCoreInstance.supplyLiquidity(10_000_000e6, marketVaultInstance.previewDeposit(10_000_000e6), 100);
        vm.stopPrank();
    ...
    function testFuzz_SupplyAndWithdrawLiquidity(uint256 amount, uint256 withdrawRatio) public {
        ...
        marketCoreInstance.supplyLiquidity(amount, expectedShares, 500); // 5% slippage
        ...
        if (withdrawShares > 0) {
            ...
            marketCoreInstance.withdrawLiquidity(withdrawShares, expectedAmount, 500);
            vm.stopPrank();
            ...
            if (borrowAmount > 0 && borrowAmount <= availableLiquidity) {
                try marketCoreInstance.borrow(positionId, borrowAmount, type(uint256).max, 1000) {
                    ...
                    if (debtRatio >= 100) {
                        ...
                        assertTrue(marketCoreInstance.isLiquidatable(bob, positionId), "Position should be liquidatable");
    ...
    function testFuzz_ProtocolConfiguration(
        ...
        assertEq(loadedConfig.flashLoanFee, flashFee);
    }
    ...
    function testFuzz_SlippageProtection(uint256 amount, uint256 slippageBps) public {
        ...
        try marketCoreInstance.supplyLiquidity(amount, minShares, uint32(slippageBps)) {
        ...
        if (slippageBps < 5000) {
            try marketCoreInstance.supplyLiquidity(amount, expectedShares * 2, uint32(slippageBps)) {
