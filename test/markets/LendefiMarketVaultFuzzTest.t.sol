...
contract LendefiMarketVaultFuzzTest is BasicDeploy {
    ...
    function setUp() public {
        ...
        marketCoreInstance.supplyLiquidity(INITIAL_LIQUIDITY, marketVaultInstance.previewDeposit(INITIAL_LIQUIDITY), 100);
        vm.stopPrank();
    }
        ...
        try marketCoreInstance.loadProtocolConfig(config) {
            ...
            if (loanAmount > 0) {
                ...
                try marketVaultInstance.flashLoan(address(flashReceiver), loanAmount, "") {
                    ...
                    assertEq(
                        usdcInstance.balanceOf(address(marketVaultInstance)),
    ...
    function testFuzz_YieldBoost(uint256 boostAmount, uint256 numBoosts) public {
        ...
        deal(address(usdcInstance), address(marketCoreInstance), totalBoost);
